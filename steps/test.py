...
def compute_score(logit_list, softmax_list, score_wgts, branch_opt):
...
def evaluation(model, test_loader, out_loader, **options):
    ...
    with torch.no_grad():
        for data, labels in test_loader:
            ...
            with torch.set_grad_enabled(False):
                ...
                softmax_list = torch.softmax(softmax_list / options['temp'], dim=2)
    ...
    micro_f1 = f1_score(total_label, ((total_pred_label + 1) * open_pred) - 1, average='micro')
    ...
    print('Accuracy: {:.3f}, '      .format(acc), 
          ...
          'Micro F1-score: {:.5f}, '.format(micro_f1), 
          'Det_ACC: {:.5f}, '       .format(det_acc),
          'FPR@TPR95: {:.5f}, '     .format(fpr_tpr95),
        )
    
    return acc, auroc, aupr_in, aupr_out, macro_f1, micro_f1, det_acc, fpr_tpr95